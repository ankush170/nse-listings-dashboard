...
export function Sidebar({ onCompanySelect }: SidebarProps) {
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 bg-[#8117DE] shadow-lg transition-all duration-300 ease-in-out z-50 ${
        isOpen ? "w-64" : "w-16"
      }`}
    ...
    >
      <div className="h-full flex flex-col">
        <div className="p-4 flex items-center">
          ...
          />
          {isOpen && (
            <span className="ml-2 text-white font-semibold">Onfinance AI</span>
          )}
        </div>
        {isOpen ? (
          <div className="p-6 flex-1 overflow-y-auto">
            ...
            <div className="space-y-4">
              {companies.map((company) => (
                <div
                  key={company.id}
                  className={`bg-gradient-to-br from-purple-400 via-purple-500 to-purple-600 rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-300 cursor-pointer ${
                    selectedCompany === company.name ? "ring-2 ring-white" : ""
                  }`}
